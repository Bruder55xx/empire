...
class PixelTod:
    ...
    def main(self):
        ...
        self.log(f'{Fore.LIGHTYELLOW_EX}Hesaplar algılandı: {len(datas)}')
        if not datas:
            self.log(f'{Fore.LIGHTYELLOW_EX}Lütfen bilgilerinizi girin initdata.txt')
            sys.exit()
        print('-' * 50)
        while True:
            for no, data in enumerate(datas):
                self.log(f'{Fore.LIGHTYELLOW_EX}Hesap numarası: {Fore.LIGHTWHITE_EX}{no + 1}')
    ...
    def process_account(self, apiKey: str, id_data):
        ...
        if offline_bonus > 0:
            ...
            if self.get_offline_bonus(apiKey=apiKey):
                self.log(f"{Fore.LIGHTYELLOW_EX}Çevrimdışı bonus: {Fore.LIGHTWHITE_EX}+{number_short(value=offline_bonus)}")
        else:
            self.log(f"{Fore.LIGHTRED_EX}Çevrimdışı bonus yok")
        ...
        self.log(f"{Fore.LIGHTYELLOW_EX}seviye: {Fore.LIGHTWHITE_EX}{self.level} | "
                 f"{Fore.LIGHTYELLOW_EX}Denge: {Fore.LIGHTWHITE_EX}+{number_short(value=self.balance)} "
                 f"{Fore.LIGHTYELLOW_EX}Saat başına gelir: {Fore.LIGHTWHITE_EX}+{number_short(value=self.mph)}")
        ...
        if daily_index is not None:
            ...
            if daily_claimed:
                self.log(f"{Fore.LIGHTYELLOW_EX}Günün ödülünü topladık")

        else:
            self.log(f"{Fore.LIGHTRED_EX}Günlük ödül mevcut değil")

        unrewarded_quests = [quest['key'] for quest in full_profile['quests'] if not quest['isRewarded']]
        if unrewarded_quests:
            self.log(f"{Fore.LIGHTYELLOW_EX}Mevcut görevler var")
            time.sleep(random.randint(2, 4))
            for quest in unrewarded_quests:
                time.sleep(random.randint(1, 2))
                if self.quest_reward(quest=quest, apiKey=apiKey):
                    self.log(f"{Fore.LIGHTYELLOW_EX}Görev ödülü {Fore.LIGHTWHITE_EX}{quest} {Fore.LIGHTYELLOW_EX}получена")
        ...
        if unrewarded_friends:
            self.log(f"{Fore.LIGHTYELLOW_EX}Arkadaşlık ödülü mevcut")

            while len(unrewarded_friends) >= 10:
                success, friends = self.friend_reward(batch=True, apiKey=apiKey)
                if success:
                    self.log(f"{Fore.LIGHTYELLOW_EX}10 arkadaşa ödül verildi")
            ...
            for friend in unrewarded_friends:
                ...
                if success:
                    self.log(f"{Fore.LIGHTYELLOW_EX}Bir arkadaşa ödül {Fore.LIGHTWHITE_EX}{friend} {Fore.LIGHTYELLOW_EX}получена")
                time.sleep(random.randint(1, 2))
        ...
        for dbQuest in self.dbData['dbQuests']:
            ...
            if not any(dbQuest['key'] in quest['key'] for quest in full_profile['quests']):
                if self.quest_reward(quest=dbQuest['key'], apiKey=apiKey):
                    quests_completed += 1
                    self.log(f"{Fore.LIGHTYELLOW_EX}Görev ödülü {Fore.LIGHTWHITE_EX}{dbQuest['key']} {Fore.LIGHTYELLOW_EX}получена")
        ...
        if need_quiz:
            ...
            self.log(f"{Fore.LIGHTYELLOW_EX}Bugün gece dönemi başlayacak {Fore.LIGHTWHITE_EX}{str(day_end)}")
            if self.level >= quiz_req_level:
                time.sleep(random.randint(2, 4))
                if self.complete_quest(quest=quiz_key, code=quiz_answer, apiKey=apiKey):

                    self.log(f"{Fore.LIGHTYELLOW_EX}Alınan günlük sınav için ödül")
        if need_rebus:
            day_start = datetime_time(random.randint(8, 9), random.randint(0, 59))
            self.log(f"{Fore.LIGHTYELLOW_EX}Yarın günlük süre şu saatte başlayacak: {Fore.LIGHTWHITE_EX}{str(day_start)}")
            if self.level >= rebus_req_level:
                time.sleep(random.randint(2, 4))
                if self.complete_quest(quest=rebus_key, code=rebus_answer, apiKey=apiKey):

                    self.log(f"{Fore.LIGHTYELLOW_EX}Alınan günlük bulmaca ödülü")
                ...
                if possible_skill is not None:
                    if self.balance - possible_skill['price'] >= config.PROTECTED_BALANCE:
                        ...
                        if improve_data is not None:
                            self.log(
                                f"{Fore.LIGHTYELLOW_EX}Madencilik Becerisi {Fore.LIGHTWHITE_EX}{possible_skill['key']} {Fore.LIGHTYELLOW_EX}yükseltildi {Fore.LIGHTWHITE_EX}{possible_skill['newlevel']}")
                        else:
                            break
        ...
        if config.SKILLS_COUNT > 0:
            ...
            while improved_skills < config.SKILLS_COUNT:
                ...
                if self.balance - skill['price'] < config.PROTECTED_BALANCE:
                    self.log(f"{Fore.LIGHTYELLOW_EX}Beceri gelişimi durduruldu (denge koruması)")
                    break
                time.sleep(random.randint(2, 4))
                ...
                if improve_data is None:
                    break
                improved_skills += 1
                self.log(f"{Fore.LIGHTYELLOW_EX}Yetenek {Fore.LIGHTWHITE_EX}{skill['key']} {Fore.LIGHTYELLOW_EX}улучшен до уровня {Fore.LIGHTWHITE_EX}{skill['newlevel']}")
            ...
            if self.dbData:
                        ...
                        if config.PVP_UPGRADE_LEAGUE:
                            for league in self.dbData['dbNegotiationsLeague']:
                                if league['requiredLevel'] > selected_league['requiredLevel'] and self.level >= league['requiredLevel']:
                                    ...
                                    break
                            self.log(
                                f"{Fore.LIGHTYELLOW_EX}Seçilen lig artık mevcut değil. Yeni lig: {Fore.LIGHTWHITE_EX}{league_data['key']}.")
                        else:
                            config.PVP_ENABLED = False
                            self.log(
                                f"{Fore.LIGHTRED_EX}Seçilen lig artık mevcut değil. PvP devre dışı")
                    else:
                        config.PVP_ENABLED = False
                        self.log(f"{Fore.LIGHTRED_EX}PVP_LEAGUE parametresi geçersiz. PvP devre dışı")
                ...
                if league_data is not None:
                    ...
                    if config.PVP_STRATEGY == 'random' or config.PVP_STRATEGY in self.strategies:
                        ...
                        config.PVP_ENABLED = False
                        self.log(f"{Fore.LIGHTRED_EX}PVP_STRATEGY parametresi geçersiz. PvP devre dışı")
            else:
                self.log(f"{Fore.LIGHTRED_EX}Veritabanı yok. PvP bu sefer atlanacak")
    ...
    def countdown(self, t):
        while t:
            ...
            print(f"{Fore.LIGHTWHITE_EX}Şu ana kadar bekliyorum {one:02}:{three:02}:{four:02} ", flush=True, end="\r")
            t -= 1
            time.sleep(1)
        ...
        while True:
            try:
                if method == 'GET':
                    ...
                    raise ValueError(f'Не desteklenen yöntem: {method}')
                ...
                self.log(f'{Fore.LIGHTRED_EX}Bağlantı bağlantı hatası!')
                break
    ...
    def get_profile(self, full: bool, apiKey: str) -> dict:
        ...
        try:
            ...
            self.log(f"{Fore.LIGHTRED_EX}Profil veri hatası: {error}")
            time.sleep(3)
            return {}

    def update_level(self, level: int) -> None:
        if self.level > 0 and level > self.level:
            self.log(f"{Fore.LIGHTYELLOW_EX}Yeni lvl alındı: {level}")
            self.level = level
    ...
    def get_offline_bonus(self, apiKey: str) -> bool:
        ...
        try:
            ...
            self.log(f"{Fore.LIGHTRED_EX}Çevrimdışı bonus hatası: {error}")
            time.sleep(3)
            return False
    ...
    def quest_reward(self, apiKey: str, quest: str, code: str = None) -> bool:
        ...
        try:
            ...
            self.log(f"{Fore.LIGHTRED_EX}Görev hatası: {str(error)}")
            return False
    ...
    def daily_quest_reward(self, apiKey: str, quest: str, code: str = None) -> bool:
        ...
        try:
            ...
            self.log(f"{Fore.LIGHTRED_EX}Günlük görev ödülü hatası: {str(error)}")
            return False
    ...
    def open_boxes(self, apiKey: str) -> None:
        ...
        try:
            ...
            self.log(f"{Fore.LIGHTRED_EX}Kutular açılırken hata oluştu: {str(error)}")
    ...
    def friend_reward(self, apiKey: str, batch: bool = False, friend: int = 0) -> tuple[bool, list]:
        ...
        try:
            ...
            self.log(f"{Fore.LIGHTRED_EX}Arkadaş ödülü hatası: {str(error)}")
            return False, []
    ...
    def complete_quest(self, quest: str, code: str, apiKey: str) -> bool:
        ...
        try:
            ...
            self.log(f"{Fore.LIGHTRED_EX}Görev tamamlama hatası: {str(error)}")
            return False
        ...
        try:
            ...
            if response.status_code in {200, 400, 401, 403}:
                ...
                if success:
                    ...
                    self.log(
                        f"{Fore.LIGHTRED_EX}Yardımcı hata: {response.status_code} {response_json.get('message', '')}")
                    return {}
        except Exception as error:
            self.log(f"{Fore.LIGHTRED_EX}Yardımcı hata: {str(error)}")
            return {}
    ...
    def get_funds_info(self, apiKey: str) -> dict:
        ...
        try:
            ...
            self.log(f"{Fore.LIGHTRED_EX}Yatırım hatası: {error}")
            time.sleep(3)
            return {}
    ...
    def invest(self, fund: str, amount: int, apiKey: str) -> None:
        ...
        if self.balance < amount:
            self.log(f"{Fore.LIGHTRED_EX}Yatırıma para yok")
            return
        if self.balance - amount < config.PROTECTED_BALANCE:
            self.log(f"{Fore.LIGHTRED_EX}Yatırımı atla (Denge koruması)")
            return
        try:
            ...
            if success:
                ...
                for fnd in response_json['data']['funds']:
                    if fnd['fundKey'] == fund:
                        ...
                        self.log(
                            f"{Fore.LIGHTYELLOW_EX}Yatırım yapıldı {Fore.LIGHTWHITE_EX}{number_short(value=amount)} {Fore.LIGHTYELLOW_EX}в {Fore.LIGHTWHITE_EX}{fund} {money_str}")
                        break

        except Exception as error:
            self.log(f"{Fore.LIGHTRED_EX}Yatırım hatası: {str(error)}")
    ...
    def improve_skill(self, skill: str, apiKey: str) -> dict | None:
        ...
        try:
            ...
            self.log(f"{Fore.LIGHTRED_EX}Beceri Yükseltme Hatası: {str(error)}")
            return None
            ...
            try:
                ...
                if success:
                    ...
                    if winner == id_data.id:
                        money += money_profit
                        self.log(f"{Fore.LIGHTYELLOW_EX}Risk altındaki tutar: {Fore.LIGHTWHITE_EX}{number_short(value=money_contract)} | "
                                    f"{Fore.LIGHTYELLOW_EX}Stratejiniz: {Fore.LIGHTWHITE_EX}{curent_strategy} | "
                                    f"{Fore.LIGHTYELLOW_EX}Rakibin stratejisi: {Fore.LIGHTWHITE_EX}{opponent_strategy} | "
                                    f"{Fore.LIGHTYELLOW_EX}KAZANMAK {Fore.LIGHTWHITE_EX}(+{number_short(value=money_profit)})")
                    else:
                        money -= money_contract
                        self.log(f"{Fore.LIGHTYELLOW_EX}Risk altındaki tutar: {Fore.LIGHTWHITE_EX}{number_short(value=money_contract)} | "
                                    f"{Fore.LIGHTYELLOW_EX}Stratejiniz: {Fore.LIGHTWHITE_EX}{curent_strategy} | "
                                    f"{Fore.LIGHTYELLOW_EX}Rakibin stratejisi: {Fore.LIGHTWHITE_EX}{opponent_strategy} | "
                                    f"{Fore.LIGHTRED_EX}KAYIP {Fore.LIGHTWHITE_EX}(-{number_short(value=money_contract)})")
            ...
            try:
                ...
                if success:
                    ...
                    if last and not taps_over:
                        self.log(f"{Fore.LIGHTRED_EX}Muslukları durdurun (enerji yok))")
                elif 'too many taps' in error:
                    taps_over = True

                if taps_over:
                    self.log(f"{Fore.LIGHTRED_EX}Dokunmaları durdurun (dokunma sınırına ulaşıldı))")
